...
def get_clumpiness():
    ...
    def load_data_pretransplant_version(patient_name, patient_ID):
        if patient_name in lp16_subjects_dict.values():
            ...
            clones_table = pd.read_pickle(r"/path_to_lp16_data/clones_table/{}_clones_table.pkl".format(patient_ID))
            clone_stats = pd.read_pickle(r"/path_to_lp16_data/clone_stats/{}_clone_stats.pkl".format(patient_ID))
    ...
    def get_tissue_sample_ids(patient_name):
        ...
        if patient_name in lp16_subjects_dict.values():
            samples_table = pd.read_pickle(r"/path_to_lp16_data/samples.pkl")
            metadata_table = pd.read_pickle(r"/path_to_lp16_data/sample_metadata.pkl")
    ...
    for patient_ID, patient_name in lp16_subjects_dict.items():
        ...
        temp_folder_where_files_will_be_stored_and_deleted = "/path_to_lp16_data/temp"
        ...
        clones_table = load_data_pretransplant_version(patient_name, patient_ID)
        tissues_per_sample, samples_per_tissue, tissues = get_tissue_sample_ids(patient_name)
        tissue_pair_clones_dict = pd.read_pickle(r"/path_to_lp16_data/{}_clones_per_tissue_pairs_new_1.pkl".format(patient_name))
        ...
        # Save results    
        # =============================================================================          
        path = Path(r"/path_to_lp16_data/{}_clumpiness_all_tissue_pair_clones_updated_node_min_script.pkl".format(patient_name))
