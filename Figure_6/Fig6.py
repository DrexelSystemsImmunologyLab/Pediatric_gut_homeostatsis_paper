...
def make_fig():
    ...
    for x_axis_name, x_axis_value in label_order.items():
        ...
        elif len(x_values)  > 1: #if there are two or more points then add jitter
    ...
    pio.write_html(fig, file=r'/path_to_output_save_folder/Fig_6.html') #produces an interactive .html graph. You may want to comment out lines "autosize=False, width=some_number, height=some_number" in fig.update_layout to allow the size to be automatically made for this file.
    fig.write_image('/path_to_output_save_folder/Fig_6.png', scale=8) #produces a high res .png image
    print("Finished!")
